...
#define INODE_BITMAP_SIZE ((MAX_FILES+7)/8) // 125 Bytes
#define INODE_BITMAP_SECTORS ((INODE_BITMAP_SIZE+SECTOR_SIZE-1)/SECTOR_SIZE) //1 Sector

// 3. the sector bitmap (one or more sectors), which indicates whether
// the particular sector in the disk is currently in use
#define SECTOR_BITMAP_START_SECTOR (INODE_BITMAP_START_SECTOR+INODE_BITMAP_SECTORS) // 2 (2nd Sector)
...
#define SECTOR_BITMAP_SIZE ((TOTAL_SECTORS+7)/8) //1250 Bytes
#define SECTOR_BITMAP_SECTORS ((SECTOR_BITMAP_SIZE+SECTOR_SIZE-1)/SECTOR_SIZE) // 3 Sectors

// 4. the inode table (one or more sectors), which contains the inodes
// stored consecutively
#define INODE_TABLE_START_SECTOR (SECTOR_BITMAP_START_SECTOR+SECTOR_BITMAP_SECTORS) // 5 (5th Sector)
...
#define INODES_PER_SECTOR (SECTOR_SIZE/sizeof(inode_t)) //1254 inodes per sector
#define INODE_TABLE_SECTORS ((MAX_FILES+INODES_PER_SECTOR-1)/INODES_PER_SECTOR) //1

// 5. the data blocks; all the rest sectors are reserved for data
// blocks for the content of files and directories
#define DATABLOCK_START_SECTOR (INODE_TABLE_START_SECTOR+INODE_TABLE_SECTORS) // 6
...
static void bitmap_init(int start, int num, int nbits)
{
  /* YOUR CODE */
  int counter = 0;
  unsigned char buf[SECTOR_SIZE] = {0}; //set all bits to 0
  int nbyte = nbits / 8; //number of bytes that will be set
  int rem_bits = nbits % 8; //number of bits
  int last_sector = start + num;

  if(nbits == 0)
  {
    for (counter = start; counter < last_sector; counter++)
    {
      //starting from the start sector set all 0's
      if(Disk_Write(counter, buf) < 0) {
  ...
  if (nbyte == 0) //nbits dont make up a byte
  {
    for(counter = 0; counter < nbits; counter++) 
    {
      buf[0] |= 1 << (counter+1); //set 'nbits' bits in the first byte of the sector to 1
    }
    if(Disk_Write(start, buf) < 0) { //write to start sector
        dprintf("... failed writing the block %d\n" , start);
        osErrno = E_GENERAL;
        return;
    }
    memset(buf, 0, SECTOR_SIZE); //reset buf
    for (counter = start+1; counter < last_sector; counter ++)
    {
      //starting from the sector after the first, set all 0's
      if(Disk_Write(counter, buf) < 0) { //write to rest of sectors
        dprintf("... failed writing the block %d\n" , counter);
        osErrno = E_GENERAL;
        return;
      }
    }
  }else{ //nbits makes up at least 1 byte
    if (nbyte >= SECTOR_SIZE)//nbyte is greater than or equal to a whole sector
    {
      int num_sectors = nbyte / SECTOR_SIZE;
      int rem_bytes = nbyte % SECTOR_SIZE;
      if(num_sectors >= 1) //there are one or more sectors to set to all 1's
      {
        memset(buf, 255, SECTOR_SIZE); //set whole sector to 1
        for (counter = start; counter < (start + num_sectors); counter++)
        {
          if(Disk_Write(counter, buf) < 0) //write to sectors
          { 
            dprintf("... failed writing the block %d\n" , start);
            osErrno = E_GENERAL;
            return;
        ...
        if(rem_bytes == 0 && rem_bits == 0)//num_sectors was exactly one sector
        {  
          memset(buf, 0, SECTOR_SIZE); //exactly one sector, everything else 0
          for(counter = start + num_sectors; counter < last_sector; counter++)
          {
            if(Disk_Write(counter, buf) < 0) //write to all sectors start + num_sectors and after
            { 
              dprintf("... failed writing the block %d\n" , start);
              osErrno = E_GENERAL;
              return;
        ...
        } else if(rem_bytes != 0 && rem_bits == 0) //remaining bytes but no remaining bits
        {
          memset(buf, 255, rem_bytes); //set rem_bytes bytes to 1
          if(Disk_Write(start + num_sectors, buf) < 0) //write to the sector start + num_sectors
          { 
            dprintf("... failed writing the block %d\n" , start);
            osErrno = E_GENERAL;
            return;
          }
          memset(buf, 0, SECTOR_SIZE); //reset buf to all 0's
          for(counter = start + num_sectors + 1; counter < last_sector; counter++)
          {
            if(Disk_Write(counter, buf) < 0) //write to all sectors after start + num_sectors
            { 
              dprintf("... failed writing the block %d\n" , start);
              osErrno = E_GENERAL;
              return;
        ...
        } else if(rem_bytes == 0 && rem_bits != 0) //remaining bits but no remaining bytes
        {
          memset(buf, 0, SECTOR_SIZE); // reset buf
          for(counter = 0; counter < rem_bits; counter++) 
          {
            buf[0] |= 1 << (counter+1); //set 'nbits' bits in the first byte of the sector to 1
          }
          if(Disk_Write(start + num_sectors, buf) < 0) //write to the sector start + num_sectors
        ...
        } else if(rem_bytes != 0 && rem_bits != 0) //remaining bytes and bits
        {
          memset(buf, 255, rem_bytes); //set rem_byte bytes of sector to all 1's
          int next_byte = rem_bytes + 1;
          for(counter = 0; counter < rem_bits; counter++)
          {
            buf[next_byte] |= 1 << (counter+1); //set rem_bits bits in the (nbyte+1)th byte of the sector to 1
          }
          if(Disk_Write(start + num_sectors, buf) < 0) //write to the sector start + num_sectors
          { 
            dprintf("... failed writing the block %d\n" , start);
      ...
      } else //nbyte doesnt make a whole sector
      {
        memset(buf, 255, nbyte); //set nbyte bytes of sector to all 1's
        int next_byte = nbyte + 1;
        for(counter = 0; counter < rem_bits; counter++) //if no rem_bits loop does nothing
        {
          buf[next_byte] |= 1 << (counter+1); //set rem_bits bits in the (nbyte+1)th byte of the sector to 1
        }
        if(Disk_Write(start, buf) < 0) //write to the start sector
        { 
          dprintf("... failed writing the block %d\n" , start);
          osErrno = E_GENERAL;
          return;
        ...
        for(counter = start + 1; counter < last_sector; counter++)
        {
          if(Disk_Write(counter, buf) < 0) //write to all sectors after start
          { 
            dprintf("... failed writing the block %d\n" , start);
            osErrno = E_GENERAL;
            return;
...
// set the first unused bit from a bitmap of 'nbytes' bytes (flip the
// first zero appeared in the bitmap to one) and return its location;
// return -1 if the bitmap is already full (no more zeros)
static int bitmap_first_unused(int start, int num, int nbytes)
{
  /* YOUR CODE */
  int counter = 0;
  int byte_counter = 0;
  ...
  for(counter = start; counter < num; counter++) //start at the start sector
  {
    if(Disk_Read(counter, buf) < 0)//read sector into buf
    {
      dprintf("... failed reading the block %d\n" , start);
      osErrno = E_GENERAL;
      return;
    }
    byte_counter = 0; //reset byte_counter before starting this loop  
    for(; byte_counter < SECTOR_SIZE; byte_counter++)
    {
      int i = 0;
      for(; i < 8; i++)
      {
        unsigned char num = (buf[byte_counter] >> i) & 1; //get bits, bit by bit, from buf[byte_counter]
        if(num == 0) //if bit free
        {
          buf[byte_counter] |= 1 << (i); //set the free bit to 1
          if(Disk_Write(counter, buf) < 0) 
          {
            dprintf("... failed writing the block %d\n" , start);
            osErrno = E_GENERAL;
          ...
          int bit_pos = ((byte_counter - 1) * 8) + i; //byte_counterth byte - 1 is all the bits already referenced + ith bit that was flipped  
          return bit_pos;
        }
      }
    }
...
{
  /* YOUR CODE */
  /* Bitmaps will take up at most 3 sectors, so first do arithmetic check to
     see if the i-th bit is in which sector                             */

  /* Bit cant be in the first sector if i-th bit > 512 bytes * 8 */
  unsigned char buf[SECTOR_SIZE];
  int bits_per_sector = SECTOR_SIZE * 8;

  if(ibit > (bits_per_sector - 1))
  {
    ibit -= (bits_per_sector - 1);
    start++; //go to the next sector
  }

  /* If i-th bit greater than 2 sectors of bits then go to third*/
  if(ibit > ((2 * bits_per_sector) - 1))
  {
    ibit -= ((2 * bits_per_sector) - 1);
    start += 2; //go to the proper sector
  }

  if(Disk_Read(start, buf) < 0)//read sector into buf
  {
    dprintf("... failed reading the block %d\n" , start);
    osErrno = E_GENERAL;
    return;
  ...
  int ibyte = ibit / 8; //find the ith byte to be referenced
  int bit_to_reset = ibit % 8;   
  
  buf[ibyte] &= ~(1 << bit_to_reset);
  unsigned char num = (buf[ibyte] >> bit_to_reset) & 1; //bit at buf[ibyte]
  if(num == 0) //if bit free  
  {
    return 0;
  }
...
static int illegal_filename(char* name)
{
  ...
  }else
  {
    ...
    {
      char check = name[i];
      if(!(isalpha(check) || isdigit(check) || check == '-' || check == '_'))//Check for valid characters
        return 1;
    }
  }
  return 0; 
